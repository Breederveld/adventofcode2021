                ...
                for (int scanner0 = 0; scanner0 < scanners.Count; scanner0++)
                {
                    ...
                    for (int scanner1 = 0; scanner1 < scanners.Count; scanner1++)
                    {
                        ...
                        if (scanner0 != scanner1)
                        {
                            ...
                            Console.WriteLine($"Found {relative.Count}");
                        }
                    }
                }
            }

            var scannerPos = relative.Select(kv => kv.Value[0]).ToArray();
            var max = 0;
            for (int scanner0 = 0; scanner0 < scannerPos.Length; scanner0++)
            {
                for (int scanner1 = 0; scanner1 < scannerPos.Length; scanner1++)
                {
                    var distance = Math.Abs(scannerPos[scanner0].x - scannerPos[scanner1].x)
                        + Math.Abs(scannerPos[scanner0].y - scannerPos[scanner1].y)
                        + Math.Abs(scannerPos[scanner0].z - scannerPos[scanner1].z);
                    if (distance > max)
                    {
                        max = distance;
                    }
        ...
        private static List<(int x, int y, int z)> FindRelative(List<(int x, int y, int z)> scanner0, List<(int x, int y, int z)> scanner1)
        {
            ...
            for (int rotX = 0; rotX < 4; rotX++)
            {
                for (int rotY = 0; rotY < 4; rotY++)
                {
                    ...
                    {
                        for (int pos0 = 1; pos0 < scanner0.Count; pos0++)
                        {
                            for (int pos1 = 1; pos1 < scanner1.Count; pos1++)
                            {
